...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        prompts_text = [
            "<|vision_start|><|image_pad|><|vision_end|>" + sample["prompt"][-1]["content"][-1]["text"]
        ]
        ...
        for i, (reward_func, reward_tok) in enumerate(zip(self.reward_funcs, self.reward_processing_classes)):
            if isinstance(reward_func, PreTrainedModel):
                ...
                reward_kwargs = {
                    ...
                    if key not in ["prompt", "completion", "keywords", "image", "cache_embeds", "image_path"]
                }
        ...
        advantages = advantages.detach()  # ✅ 不参与反传

        # === loss ===
        per_token_loss = -(advantages.view(-1, 1) * per_token_logps - self.beta * per_token_kl)
                ...
                json.dump(
                    {
                        "trajectories": [
                            {
                                "messages": {"prompt": prompts[0][0], "response": completions[i]},
