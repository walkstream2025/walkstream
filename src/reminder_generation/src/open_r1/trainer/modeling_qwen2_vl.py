    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if inputs_embeds is None:
            ...
            # -------------------- MOD: 视觉处理加入 cache_embeds --------------------
            if pixel_values is not None:
                # batch==1 前提：image_grid_thw 可能包含 1 行（仅真实图）或 2 行（第一行为真实图，第二行为缓存块）
                # 若提供 cache_embeds，则只用第一行 grid 走视觉编码器，把 cache 直接拼接在视觉 token 之后
                if cache_embeds is not None:
                    if image_grid_thw is None or image_grid_thw.dim() != 2 or image_grid_thw.size(0) < 1:
                        raise ValueError("When `cache_embeds` is provided, `image_grid_thw` must have at least one row for the real image.")
                    grid_real = image_grid_thw[0:1, :]  # 仅真实图
                    image_embeds_real = self.get_image_features(pixel_values, grid_real)
                    image_embeds_real = torch.cat(image_embeds_real, dim=0)

                    # 规范化 cache_embeds 形状：接受 [K,H] 或 [1,K,H]
                    if cache_embeds.dim() == 3:
                        # assume [1, K, H] 或 [B, K, H]，batch==1取第0个
                        cache_cat = cache_embeds[0]
                    elif cache_embeds.dim() == 2:
                        cache_cat = cache_embeds
                    else:
                        raise ValueError(f"`cache_embeds` must have shape [K,H] or [1,K,H], got {cache_embeds.shape}.")
                    ...
                    # 原始路径：没有 cache 时，直接按 image_grid_thw 编码所有图片
                    image_embeds = self.get_image_features(pixel_values, image_grid_thw)
                    image_embeds_all = torch.cat(image_embeds, dim=0)
...
class Qwen2VLForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # 仅在首步保留像素/缓存，后续步移除（与 pixel_values 行为一致）
        if model_inputs["cache_position"][0] != 0:
            model_inputs["pixel_values"] = None
            model_inputs["pixel_values_videos"] = None
            # -------------------- MOD: 后续步移除 cache_embeds --------------------
