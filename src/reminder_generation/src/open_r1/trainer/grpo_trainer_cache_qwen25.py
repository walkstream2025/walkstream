...
def generation_cache_mode(model):
    """
    临时切换模型到 eval 模式，关闭 gradient checkpointing，
    强制开启 use_cache，在生成阶段更稳定。
    """
...
class Qwen2_5VLGRPOTrainer(Trainer):
    """
    完整版 Qwen2.5-VL GRPO Trainer。
    基于 per-sample cache_embeds 机制。
    """
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            if args.model_init_kwargs is not None:
                raise ValueError("`model_init_kwargs` passed but model is already instantiated.")
        ...
        if processing_class is None:
            if "Qwen2.5-VL" in model_id or "Qwen2_5-VL" in model_id or "Aria" in model_id:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None,**kwargs):
        ...
        if cache_embeds.dim() not in (2, 3):
            raise ValueError(f"cache_embeds must be [K,H] or [1,K,H], got {tuple(cache_embeds.shape)}")
        if cache_embeds.dim() == 3:
            cache_embeds = cache_embeds[0]  # 规范到 [K, H]
        ...
        # === 强校验 processor 产物 ===
        if "pixel_values" not in prompt_inputs or prompt_inputs["pixel_values"] is None:
            raise ValueError("[Trainer] processor did not return pixel_values; check images list.")
        if "image_grid_thw" not in prompt_inputs or prompt_inputs["image_grid_thw"] is None:
            raise ValueError("[Trainer] processor did not return image_grid_thw; check prompts_text & images.")
        if prompt_inputs["image_grid_thw"].dim() != 2 or prompt_inputs["image_grid_thw"].size(0) < 1:
            raise ValueError(f"[Trainer] invalid image_grid_thw shape: {tuple(prompt_inputs['image_grid_thw'].shape)}")

        # === 插入 K 个 <image> token，image_grid_thw 末尾追加 [K,s,s] ===
        ...
        # 只追加 1 行 cache 网格 [K, s, s]（不覆盖真实图）
        prompt_inputs["image_grid_thw"] = self._append_cache_grid(prompt_inputs["image_grid_thw"], K)
        prompt_inputs["cache_embeds"] = cache_embeds  # dtype/device 已对齐

        # 逐次生成 G 次
        from transformers.generation.utils import GenerationConfig

        # 推断 spatial_merge_size（Trainer 有则用，没有就从模型里取）
        ...
        if spatial_merge_size is None:
            try:
                ...
                spatial_merge_size = 2  # 兜底；大多数 Qwen2-VL 默认是 2

        def _count_visual_tokens_from_grid(grid_row, s):
            # grid_row: [t,h,w] -> visual tokens after merger = t*(h*w)/(s*s)
            t, h, w = int(grid_row[0].item()), int(grid_row[1].item()), int(grid_row[2].item())
            return int(t * (h * w) // (s * s))

        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            # 文本 tokenizer 左填充，避免 FA2 对右填充的限制
            try:
        ...
        for gi in range(self.num_generations):
            ...
            # === 一致性自检：<image> token 数是否 = 真实图视觉 token 数 + K(cache) ===
            grid = prompt_inputs["image_grid_thw"]
            if grid.size(0) < 2:
                raise ValueError(f"[Trainer] image_grid_thw missing cache row after append, shape={tuple(grid.shape)}")
            num_tokens_real = _count_visual_tokens_from_grid(grid[0], spatial_merge_size)
            # 约定：最后一行是 cache 行 [K, s, s]，它映射的视觉 token 数应恰好 = K
            num_tokens_cache = _count_visual_tokens_from_grid(grid[-1], spatial_merge_size)
            if num_tokens_cache != K:
                raise ValueError(
                    f"[Trainer] cache grid row does not map to K tokens: expect {K}, got {num_tokens_cache}, "
                    f"grid_cache={grid[-1].tolist()}, s={spatial_merge_size}"
                )
            n_img_tokens = (seq_ids == self.image_token_id).sum().item()
            expected_image_tokens = num_tokens_real + K
            if n_img_tokens != expected_image_tokens:
                raise ValueError(
                    f"[Trainer] <image> token count mismatch: in-seq={n_img_tokens}, "
                    f"expected real({num_tokens_real}) + cache({K}) = {expected_image_tokens}. "
                    f"Hint: check _insert_after_image_block() and image_grid_thw appending."
                )
            # === /一致性自检 ===
        ...
        # 与你提供版本一致的 loss（不基于 ref_ratio，而是 self-normalized 重要性权重）
