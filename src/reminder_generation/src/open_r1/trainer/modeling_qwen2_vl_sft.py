...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # ====== MOD: 外部 cache 特征（视觉编码后的 embedding）拼接到 image_embeds ======
        if cache_embeds is not None:
            cache_embeds = cache_embeds.to(inputs_embeds.device, inputs_embeds.dtype)
            if image_embeds is not None:
                image_embeds = torch.cat([image_embeds, cache_embeds], dim=0)
            else:
                image_embeds = cache_embeds
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        outputs: Qwen2VLModelOutputWithPast = self.model(
            ...
            return_dict=True,
            cache_position=cache_position,
            cache_embeds=cache_embeds,  # ✅ 透传 cache_embeds
            **kwargs,
        )
